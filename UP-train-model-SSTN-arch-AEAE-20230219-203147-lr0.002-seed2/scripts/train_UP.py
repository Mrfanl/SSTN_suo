...
parser.add_argument('--sample', type=int, default=1282, help='sample sizes for training')
...
args.save = 'UP-train-model-{}-arch-{}-{}-lr{}-seed{}'.format(args.model, args.phi, time.strftime("%Y%m%d-%H%M%S"), args.learning_rate,args.seed)
...
sample_1282= [199,559,63,92,40,151,40,110,28]

def rsampling(groundTruth, sample_num = sample_1282):              #divide dataset into train and test datasets
    labels_loc = {}
    labeled = {}
    test = {}
    m = np.max(groundTruth)
