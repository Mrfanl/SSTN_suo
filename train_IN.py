...
import sys
sys.path.append('/data/Fanliang/suo_work/SSTN')    # add the SSTN root path to environment path
...
#         indices = [j for j, x in enumerate(groundTruth.ravel().tolist()) if x == i + 1]
#         np.random.shuffle(indices)
#         labels_loc[i] = indices #分类为i的所在的位置
...
def rsampling(groundTruth, sample_num = sample_512):              #divide dataset into train and test datasets
    labels_loc = {}
    labeled = {}
    test = {}
    m = np.max(groundTruth)
...
whole_data = data.reshape(data_IN.shape[0], data_IN.shape[1],data_IN.shape[2]) # (200,145,145)
#whole_data = whole_data - np.mean(whole_data, axis=(1,2), keepdims=True) 
padded_data = zeroPadding_3D(whole_data, PATCH_LENGTH) #(200,153,153)


#CATEGORY = 9

train_data = np.zeros((TRAIN_SIZE, INPUT_DIMENSION_CONV, 2*PATCH_LENGTH + 1, 2*PATCH_LENGTH + 1)) #(200,200,9,9)
test_data = np.zeros((TEST_SIZE, INPUT_DIMENSION_CONV, 2*PATCH_LENGTH + 1, 2*PATCH_LENGTH + 1)) #(9449,200,9,9)
